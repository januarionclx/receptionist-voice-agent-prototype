# AI Receptionist Prototype - Unified Implementation Plan
...
This unified plan combines the strategic vision of PROTOTYPE.md with the tactical implementation details of CLAUDE_PROTOTYPE.md, incorporating all architectural decisions and clarifications from our discussions.
...
6. **Models:** `gpt-4o-realtime-mini` (voice), `gpt-4o-mini` (supervisor)
7. **Debugging:** Include function calls panel and event logging (like Twilio demo)
8. **Session Management:** Session endpoint for tracking, but allow direct WebSocket connection
...
# Server
cd server
npm install @openai/agents ws zod openai
npm install --save-dev @types/ws

# Client
cd app
npm install zod
# No additional packages needed (native WebSocket)
```
...
#### 1.3 Create Receptionist Agent Configuration
...
- For complex queries (appointments, availability, detailed pricing), use the getNextResponseFromSupervisor tool
...
When you need the supervisor's help, call getNextResponseFromSupervisor with the conversation history.
`;
...
export async function getSupervisorResponse(
  ...
  const messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = [
    {
      role: 'system',
      content: `You are the supervisor agent for Prime Auto Lab's AI receptionist system.
...
Return responses that the receptionist can speak verbatim to the caller. Keep responses brief (2-3 sentences max) and conversational.`,
  ...
  // Initial completion with tool calling
  ...
  // Handle tool calls (can be multiple rounds)
...
#### 1.5 Implement Realtime Session Service
...
import { getSupervisorResponse } from '../agents/supervisor';
...
export async function createRealtimeSession(
  ...
  session.on('event', async (event) => {
    ...
    // Handle supervisor tool calls
    if (event.type === 'response.function_call_arguments.done') {
      if (event.name === 'getNextResponseFromSupervisor') {
        try {
          const args = JSON.parse(event.arguments);
          const response = await getSupervisorResponse(args.conversationHistory);
...
#### 1.6 Update Session Endpoint (Optional)
...
// Note: OpenAI API key is used server-side when creating RealtimeSession
router.post('/', (req, res) => {
...
import { createRealtimeSession, cleanupSession } from '../services/realtimeSession';
...
export function setupWebSocketServer(server: Server, apiKey: string) {
  ...
  wss.on('connection', async (clientWs: WebSocket, req) => {
    ...
    // Extract sessionId from query or generate new one
    ...
    try {
      ...
      // Forward audio/data from client to OpenAI
      clientWs.on('message', async (data: Buffer) => {
        const session = getSession(sessionId);
        if (session && session.status === 'active') {
          // Handle binary audio data or JSON events
          if (data[0] === 0x7b) { // JSON (starts with '{')
            const event = JSON.parse(data.toString());
            // Forward to OpenAI session
            await session.openaiSession.sendEvent(event);
          } else {
            // Binary audio data
      ...
      clientWs.on('error', (error) => {
        console.error('Browser WS error:', error);
        cleanupSession(sessionId);
      });
    } catch (error) {
...
#### 2.2 Create WebSocket Connection Hook
...
2. Client → Server: WebSocket connection (ws://localhost:3001/ws)
3. Server → OpenAI: WebSocket connection with API key auth
   (via @openai/agents SDK)
4. Server: Bidirectional relay established
5. Client ↔ Server ↔ OpenAI: Audio and events relayed
...
- `reference/openai-realtime-agents/src/app/agentConfigs/chatSupervisor/` - Agent configuration
...
1. **No Database:** Sessions are in-memory only
